        ...
        if (count > 0) {
          if (node.lastRow) {
            node.isWeight
              ? (document.getElementById(
                ...
                ).className += " node-weight-second-visited-last-row")
              : (document.getElementById(
                  `node-${node.row}-${node.col}`
                ).className += " node-second-visited-last-row");
          }
          ...
          if (node.lastCol) {
            node.isWeight
              ? (document.getElementById(
                  `node-${node.row}-${node.col}`
                ).className += " node-weight-second-visited-last-col")
              : (document.getElementById(
                  `node-${node.row}-${node.col}`
                ).className += " node-second-visited-last-col");
          }

          node.isWeight
            ? (document.getElementById(
                `node-${node.row}-${node.col}`
              ).className += " node-weight-second-visited")
            : (document.getElementById(
                `node-${node.row}-${node.col}`
              ).className += " node-second-visited");
        } else {
          if (node.lastRow) {
            node.isWeight
              ? (document.getElementById(
                  `node-${node.row}-${node.col}`
                ).className += " node-weight-first-visited-last-row")
              : (document.getElementById(
                  `node-${node.row}-${node.col}`
                ).className += " node-first-visited-last-row");
          }
          ...
          if (node.lastCol) {
            node.isWeight
              ? (document.getElementById(
                  `node-${node.row}-${node.col}`
                ).className += " node-weight-first-visited-last-col")
              : (document.getElementById(
                  `node-${node.row}-${node.col}`
                ).className += " node-first-visited-last-col");
          }

          node.isWeight
            ? (document.getElementById(
                `node-${node.row}-${node.col}`
              ).className += " node-weight-first-visited")
            : (document.getElementById(
                `node-${node.row}-${node.col}`
              ).className += " node-first-visited");
        }
      }, 10 * i);
    }
  }
    ...
    setTimeout(() => {
      ...
      if (node.isWeight) {
        document
          ...
          .getElementsByTagName("div")[0].className +=
          " node-weight-shortest-path";
      }

      if (node.lastRow) {
        document.getElementById(`node-${node.row}-${node.col}`).className +=
          " node-shortest-path-last-row";
      }

      if (node.lastCol) {
        document.getElementById(`node-${node.row}-${node.col}`).className +=
          " node-shortest-path-last-col";
      }
...
export default async function visualizeAlgorithm(
  algorithm,
  grid,
  startNode,
  finishNode,
  ...
  const firstVisitedNodesInOrder = interNode
    ? algorithm(firstGrid, startNodeObj, firstInterNodeObj)
    : algorithm(firstGrid, startNodeObj, finishNodeObj);
  let secondVisitedNodesInOrder = interNode
    ? algorithm(secondGrid, secondInterNodeObj, finishNodeObj)
    : [];
  ...
  let secondShortestPathNodes = interNode
    ? getShortestPathNodes(secondInterNodeObj, finishNodeObj)
    : [];

  if (
    secondVisitedNodesInOrder.length &&
    !firstVisitedNodesInOrder.includes(firstInterNodeObj)
  ) {
    secondVisitedNodesInOrder = [];
    secondShortestPathNodes = [];
  }
