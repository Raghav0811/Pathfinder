...
export default function useMaze() {
  ...
  const generateMaze = (grid, mazeWalls) => {
    ...
    for (let row = 1; row <= maze.length - 2; row++) {
      // guarantee four spaces in each row
      const spaces = [
        Math.ceil(Math.random() * 11),
        Math.ceil(11 + Math.random() * 11),
        Math.ceil(22 + Math.random() * 11),
        Math.ceil(33 + Math.random() * 10),
      ];
    ...
    for (let col = 1; col <= maze[0].length - 2; col++) {
      // guarantee two spaces in each column
      const spaces = [
